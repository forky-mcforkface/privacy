...
class AttackInputDataTest(parameterized.TestCase):
  ...
  def test_get_xe_loss_from_logits_with_sample_weights(self):
    ...
    # Test losses do not use sample weights.
    np.testing.assert_allclose(
        attack_input.get_loss_test(), [0.29860897, 0.95618669], atol=1e-7)
  ...
  def test_get_binary_xe_loss_from_logits_with_sample_weights(self):
    attack_input = AttackInputData(
        ...
        labels_train=np.zeros((5,)),
        labels_test=np.ones((5,)),
        sample_weight=np.array([1.0, 0.0, 0.5, 0.2, 1.0]),
        loss_function_using_logits=True)
    expected_train_loss = np.array([0.000045398, 0.0, 0.34657359125, 1.0, 10.0])
    expected_test_loss = np.array(
        [0.000045398, 0.006715348, 0.6931471825, 5, 10])
    np.testing.assert_allclose(
        attack_input.get_loss_train(), expected_train_loss, rtol=1e-2)
    # Test losses do not use sample weights.
    np.testing.assert_allclose(
        attack_input.get_loss_test(), expected_test_loss[::-1], rtol=1e-2)
