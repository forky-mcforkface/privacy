...
class AttackInputData:
  ...
  def get_loss_test(self):
    ...
    return utils.get_loss(
        ...
        # If sample weights are provided for the training
        # samples, the the sample weights for the test samples
        # should be 1.0, otherwise set them to None.
        1.0 if self.sample_weight is not None else None)
  ...
  def has_sample_weights(self):
    ...
    return self.sample_weight is not None
  ...
  def __str__(self):
    ...
    _append_array_shape(self.sample_weight, 'sample_weight', result)
    result.append(')')
    return '\n'.join(result)
